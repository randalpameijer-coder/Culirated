...
const T: Record<string, any> = {
  en: {
    topbar: "👨‍🍳  RECIPES MADE BY PEOPLE  ·  ✦ CURATED BY AI  ·  ONLY THE BEST GOES LIVE",
    ...
    filters: ["Newest", "Highest Score", "Quick"], loadMore: "Load more recipes",
  ...
  nl: {
    topbar: "👨‍🍳  RECEPTEN GEMAAKT DOOR MENSEN  ·  ✦ GEKWALIFICEERD DOOR AI  ·  ALLEEN HET BESTE GAAT LIVE",
    ...
    ctaSub: "Stuur je recept in als tekst, foto, PDF of URL. AI controleert automatisch of het klopt. Het recept blijft van jou.",
    ...
    featDesc: "Ingediend door een thuiskok en goedgekeurd door AI — verhoudingen, stappenlogica en bereidingstijd zijn allemaal gecheckt.",
  ...
  de: {
    ...
    featDesc: "Von einem Hobbykoch eingereicht und von der KI genehmigt — Mengen, Schrittlogik und Zubereitungszeit wurden alle geprüft.",
  ...
  fr: {
    topbar: "👨‍🍳  RECETTES CRÉÉES PAR DES HUMAINS  ·  ✦ SÉLECTIONNÉES PAR L'IA  ·  SEUL LE MEILLEUR EST PUBLIÉ",
...
function AIBadge({ score }: { score: number }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "5px", background: score >= 95 ? "linear-gradient(135deg,#2d5a27,#4a8c41)" : "linear-gradient(135deg,#5a4a27,#8c7a41)", borderRadius: "20px", padding: "4px 10px", fontSize: "11px", fontFamily: "monospace", color: "#e8f5e6", letterSpacing: "0.5px" }}>
...
function RecipeCard({ recipe, featured, lang, t }: { recipe: any; featured?: boolean; lang: string; t: any }) {
  ...
  return (
    <div style={{ borderRadius: "16px", overflow: "hidden", background: "#faf8f3", border: "1px solid rgba(180,160,120,0.2)", position: "relative", transition: "transform 0.3s, box-shadow 0.3s", cursor: "pointer", ...(featured ? { gridColumn: "span 2", display: "grid", gridTemplateColumns: "1fr 1fr" } : {}) }}>
      <div style={{ position: "relative", height: featured ? "100%" : "220px", minHeight: featured ? "280px" : "auto" }}>
      ...
      <div style={{ padding: featured ? "32px" : "20px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        <div>
          <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "10px" }}>
            ...
            {tags.slice(0, featured ? 3 : 1).map((tag: string) => (
          ...
          <h3 style={{ fontFamily: "Georgia, serif", fontSize: featured ? "28px" : "18px", fontWeight: "700", color: "#1e1609", lineHeight: 1.25, marginBottom: "8px" }}>{title}</h3>
          {featured && <p style={{ color: "#8a7355", fontSize: "15px", lineHeight: 1.6, marginBottom: "16px" }}>{t.featDesc}</p>}
        </div>
...
export default function Home() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f0e8", color: "#1e1609" }}>
      {/* Top bar */}
      <div style={{ background: "#1e1609", padding: "8px 0", textAlign: "center" }}>
        <span style={{ color: "#c8b080", fontFamily: "monospace", fontSize: "11px", letterSpacing: "1.2px" }}>{t.topbar}</span>
      </div>

      {/* Nav */}
      <nav style={{ background: "rgba(245,240,232,0.97)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(180,160,120,0.25)", position: "sticky", top: 0, zIndex: 100, padding: "0 48px" }}>
        <div style={{ maxWidth: "1280px", margin: "0 auto", display: "flex", alignItems: "center", height: "70px", gap: "24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", flexShrink: 0 }}>
            ...
            <span style={{ fontFamily: "Georgia, serif", fontWeight: "900", fontSize: "22px", color: "#1e1609", letterSpacing: "-0.5px" }}>Culirated</span>
          </div>
          <div style={{ display: "flex", gap: "2px", flex: 1 }}>
            {t.nav.map((item: string, i: number) => (
              <button key={i} onClick={() => setNav(i)} style={{ background: activeNav === i ? "rgba(74,122,61,0.12)" : "transparent", border: "none", borderRadius: "8px", padding: "8px 13px", cursor: "pointer", fontFamily: "monospace", fontSize: "12px", color: activeNav === i ? "#2d5a27" : "#6b5840", fontWeight: activeNav === i ? "500" : "400", whiteSpace: "nowrap" }}>{item}</button>
            ))}
          </div>
          ...
          {!isEN && (
            <button onClick={() => setLang("en")} style={{ display: "flex", alignItems: "center", gap: "7px", flexShrink: 0, background: "rgba(30,22,9,0.05)", border: "1px solid rgba(180,160,120,0.3)", borderRadius: "20px", padding: "7px 14px", cursor: "pointer", fontFamily: "monospace", fontSize: "12px", color: "#4a3820", whiteSpace: "nowrap" }}>
              🇬🇧 English
            </button>
          )}
          {isEN && localLang !== "en" && (
            <button onClick={() => setLang(localLang)} style={{ display: "flex", alignItems: "center", gap: "7px", flexShrink: 0, background: "rgba(30,22,9,0.05)", border: "1px solid rgba(180,160,120,0.3)", borderRadius: "20px", padding: "7px 14px", cursor: "pointer", fontFamily: "monospace", fontSize: "12px", color: "#4a3820", whiteSpace: "nowrap" }}>
              {LOCAL_NAMES[localLang]}
            </button>
          )}
          <button style={{ background: "#3a7a32", color: "#e8f5e6", border: "none", borderRadius: "24px", padding: "10px 18px", cursor: "pointer", fontFamily: "monospace", fontSize: "12px", fontWeight: "500", whiteSpace: "nowrap", flexShrink: 0 }}>{t.submit}</button>
        </div>
      </nav>

      {/* Hero */}
      <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "72px 48px 56px", display: "grid", gridTemplateColumns: "1fr 440px", gap: "64px", alignItems: "center" }}>
        <div>
          ...
          <div style={{ display: "flex", marginBottom: "36px", background: "rgba(30,22,9,0.04)", borderRadius: "14px", border: "1px solid rgba(180,160,120,0.2)", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", height: "480px" }}>
          <div style={{ position: "absolute", top: "20px", right: "0", width: "350px", height: "400px", borderRadius: "24px", overflow: "hidden", boxShadow: "0 30px 80px rgba(30,22,9,0.25)" }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          <div style={{ position: "absolute", bottom: "10px", left: "0", width: "210px", height: "175px", borderRadius: "16px", overflow: "hidden", boxShadow: "0 20px 40px rgba(30,22,9,0.2)", border: "4px solid #f5f0e8" }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          <div style={{ position: "absolute", top: "16px", left: "0", background: "#fff", borderRadius: "16px", padding: "16px 20px", boxShadow: "0 12px 32px rgba(30,22,9,0.15)", border: "1px solid rgba(180,160,120,0.2)", minWidth: "165px" }}>
      ...
      {/* Categories */}
      <div style={{ background: "#1e1609", padding: "40px 48px", marginBottom: "64px" }}>
      ...
      {/* How it works */}
      <div style={{ background: "linear-gradient(135deg,#2d5a27,#1a3d16)", padding: "80px 48px" }}>
        <div style={{ maxWidth: "1280px", margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "80px", alignItems: "center" }}>
          <div>
            ...
            <h2 style={{ fontFamily: "Georgia, serif", fontSize: "48px", fontWeight: "700", color: "#e8f5e4", lineHeight: 1.05, marginBottom: "24px" }}>
      ...
      <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "72px 48px" }}>
        <div style={{ background: "#1e1609", borderRadius: "28px", padding: "64px", display: "grid", gridTemplateColumns: "1fr auto", gap: "48px", alignItems: "center" }}>
          <div>
            <h2 style={{ fontFamily: "Georgia, serif", fontSize: "44px", color: "#e8dfc8", marginBottom: "16px", letterSpacing: "-0.5px" }}>
      ...
      {/* Footer */}
      <footer style={{ borderTop: "1px solid rgba(180,160,120,0.2)", padding: "40px 48px" }}>
