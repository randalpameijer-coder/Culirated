...
// Keep this in sync met T.en in app/page.tsx
// Versie ophogen = alle Supabase caches worden automatisch gewist en opnieuw vertaald
const VERSION = "v5";
...
async function translateFresh(targetLang: string) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Check Supabase cache — versie zit als _v veld in de strings JSON zelf
    ...
    if (cached?.strings && cached.strings._v === VERSION) {
      return NextResponse.json(cached.strings);
    }

    // 2. Cache ontbreekt of verouderd — vertaal opnieuw
    const translated = await translateFresh(targetLang);
    // Sla versienummer op in de strings zelf
    translated._v = VERSION;

    // 3. Sla op in Supabase
    await supabase
      .from("ui_translations")
      .upsert({ lang: targetLang, strings: translated, updated_at: new Date().toISOString() });
